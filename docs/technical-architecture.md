...
- 同步 HTTP 接口只做快速操作：鉴权、参数校验、轻量读写、创建任务、查询状态。
- 任何依赖外部模型、联网搜索、URL 抓取、资料清洗、证据抽取、人物画像合成、批量处理的动作，都必须进入后台任务。
- API 不等待后台任务完成，只返回稳定的 `jobId`、`status`、`nextAction` 和前端可继续轮询或订阅的状态入口。
- 后台任务由 worker 消费，负责 claim、heartbeat、retry、fail、succeed 和终态落库。
- 前端通过 polling、SSE、WebSocket 或平台消息能力接收完成结果；V1 可先用 polling，后续再升级实时通知。
- 上游模型或搜索服务的临时错误不能被包装成普通业务 `400`；应落到任务失败状态，并保留可重试原因。

适用到一键蒸馏：

- `source discovery` 属于长耗时任务，因为它依赖 Kimi web search 和外部网络，不应该同步阻塞创建流程。
- API 目标行为是创建 `sourceDiscoveryJob` 后立即返回，由后台完成资料搜索和候选资料写入。
- 用户确认资料后，再创建 `distillJob`；`distillJob` 继续由 worker 异步完成画像合成。
- 只有聊天这类用户直接等待的低延迟交互，才允许保持同步接口，但也必须有超时和降级策略。
